...
in a similar way int variables would be swapped*/
...
void power_matrix(int ***mat, int n, int k)
{
	...
	while (k > 0) {
		if (k % 2 == 1) {//power is an odd number;
			for (int i = 0; i < n; ++i)
				for (int j = 0; j < n; ++j) {
					for (int l = 0; l < n; ++l) {
...
void command_C(int ****container, int *ln, int *cn, int cnt)
{
	...
	if (cnt < index + 1 || index < 0) {
		...
		int rows_C, columns_C;//rows and columns new matrix has
		scanf("%d", &rows_C);
...
//orders matrices in an ascending way, by the sum of the matrix elements;
...
//function that behaves like main;
	...
	while (*c != 'Q') {
		if (*c == 'L') {
			...
			if (*cnt < index + 1 || index < 0) {
				...
				free_mat((*container)[index], rows[index]); //frees matrix
